...
```bash
git -C /home/israel/Documents/Development/PlantApp rev-parse HEAD   # must be 59288c6a51a6569d7d6a7fec361b25897c4f1b58
...
```
New imports needed in `AddPlantWizard.kt` (these are the ones VERIFIED ABSENT;
`Row`, `Icon`, and `rememberScrollState` are already imported — skip them):
...
`androidx.compose.material3.FilterChip`, `androidx.compose.material3.FilterChipDefaults`,
`androidx.compose.material3.OutlinedTextField`, and — IMPORTANT —
`import dev.plantapp.feature.inventory.DisplayText` (the wizard lives in the
`…feature.inventory.addplant` sub-package, so `DisplayText` is NOT in the same
package and the import is required).

FilterChip color: the Hearth spec asks `primaryContainer` — pass
`colors = FilterChipDefaults.filterChipColors(selectedContainerColor =
...
```kotlin
    ...
    fun `species step filters tiles by search text`() {
        composeRule.setContent {
            AddPlantWizard(
                profiles = profiles,   // the file's existing class-level fixture list
...
```
Fixture note: `AddPlantWizardTest` already holds a class-level `profiles` list
built with positional `PlantProfile(id, scientificName, listOf(common), category)`
constructor calls — REUSE it (it contains tomato + basil); if it doesn't cover
the case, extend it in the same positional style. Do NOT invent a helper.
IMPORT note: the test file does NOT yet import
`androidx.compose.ui.test.performTextInput` — add it with the test, otherwise
the red step fails as a compile error instead of the expected assertion red.
